...
class BoxShape(x: Float, y: Float, z: Float) : btBoxShape(Vector3(x, y, z))
class CylinderShape(x: Float, y: Float, z: Float) : btCylinderShape(Vector3(x, y, z))

class Mass(var value: Float)

class PhysicalAttributes(val shape: btCollisionShape, val mass: Mass = Mass(0f), val collidesWith: CollisionMasks)

abstract class AbstractPhysicalEntity(modelResource: Resource<Model>, val physics: PhysicalAttributes) : AbstractEntity(modelResource) {
    ...
    init {
        if (physics.mass.value > 0f) {
            physics.shape.calculateLocalInertia(physics.mass.value, localInertia)
        } else {
            localInertia.set(0f, 0f, 0f)
        }
        //motionState = MotionState()
        constructorInfo = btRigidBody.btRigidBodyConstructionInfo(physics.mass.value, null, physics.shape, localInertia)
